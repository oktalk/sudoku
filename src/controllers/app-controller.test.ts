...
describe('validate board', () => {
  ...
  test('check a row for 8, value should be present', () => {
    board[3][3] = 8;
    expect(checkRow(3, 8, board)).toEqual({ row: 3, col: 3 })
  });
  ...
  test('check column for 8, value should be present', () => {
    board[3][3] = 8;
    expect(checkCol(3, 8, board)).toEqual({ row: 3, col: 3 });
  });
  ...
  test('check block for 8, value should be present', () => {
    board[5][4] = 8;
    expect(checkBlock(3, 3, 8, board)).toEqual({ row: 5, col: 4 });
  });
  test('check board, if any value is 0, return false', () => {
  ...
  test('check board, if invalid return false', () => {
    board = [
      [5, 3, 4, 6, 7, 8, 9, 1, 2],
      [6, 7, 2, 1, 9, 0, 3, 4, 8],
      [1, 0, 0, 3, 4, 2, 5, 6, 0],
